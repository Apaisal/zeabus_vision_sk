...
def create_trackbar():
	...
	cv2.moveWindow('yellow',width/2,(height/3)+50)
	cv2.moveWindow('orange',width/2,(height*2/3)+50)
...
def select_color():
	...
	while not rospy.is_shutdown():
		# hsv = rebalance(img.copy())
		hsv = process_img_top(img.copy())
		# hsv = equalization(img.copy())
		# bgr = cv2.cvtColor(hsv,cv2.COLOR_HSV2BGR)
		# hsv =  clahe(bgr)
		cv2.imshow('img_bgr',cv2.cvtColor(hsv,cv2.COLOR_HSV2BGR))
...
def callback(msg):
	...
	if wait == False:
		...
		img = cv2.resize(cv2.imdecode(arr,1),(320,256))
...
class Color:
	def __init__(self):
		...
		self.name = 'color_top'
		self.upper_yellow = rospy.get_param("color_range/color_top/upper_yellow",[0,0,0])
		self.lower_yellow = rospy.get_param("color_range/color_top/lower_yellow",[180,255,255])
		self.upper_orange = rospy.get_param("color_range/color_top/upper_orange",[0,0,0])
		self.lower_orange = rospy.get_param("color_range/color_top/lower_orange",[180,255,255])
...
class Finding_Color:
	...
	def save(self):
		try :
			for i in range (0,3):
				rospy.set_param('/color_range/color_top/lower_orange',self.color.lower_orange)
				rospy.set_param('/color_range/color_top/upper_orange',self.color.upper_orange)
		
				rospy.set_param('/color_range/color_top/lower_yellow',self.color.lower_yellow)
				rospy.set_param('/color_range/color_top/upper_yellow',self.color.upper_yellow)
...
if __name__ == '__main__':
	rospy.init_node('color_range_top')
	topic = rospy.get_param('/color_range/topic_top','/rightcam_top/image_raw/compressed')
	# topic = rospy.get_param('/color_range/topic_top','/leftcam_top/image_raw/compressed')
	rospy.Subscriber(topic,CompressedImage,callback,queue_size=1)
	select_color()
	rospy.spin()